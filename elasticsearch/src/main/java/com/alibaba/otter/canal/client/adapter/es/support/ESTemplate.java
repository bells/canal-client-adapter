    ...
    public void insert(ESMapping mapping, Object pkVal, Map<String, Object> esFieldData) {
        if (mapping.get_id() != null) {
            ...
            try {
                ...
                logger.error("search or update error: ", e);
            }
        }
...
//            }
    ...
    private String getEsType(ESMapping mapping, String fieldName) {
        ...
        if (fieldType == null) {
            Map<String, MappingMetaData> allMappings;
            try {
                GetMappingsRequest request = new GetMappingsRequest().indices(mapping.get_index());
                GetMappingsResponse getMappingResponse = restHighLevelClient.indices().getMapping(
                        request, RequestOptions.DEFAULT);
                if (getMappingResponse == null) {
                    logger.error("get index mapping info exception. index: " + mapping.get_index());
                    throw new IllegalArgumentException("Not found the mapping info of index: " + mapping.get_index());
                }
                allMappings = getMappingResponse.mappings();
            } catch (IOException e) {
                logger.error("get index mapping info exception. index: " + mapping.get_index(), e);
                throw new RuntimeException("get index mapping info exception. index: " + mapping.get_index());
            }

            MappingMetaData mappingMetaData = allMappings.get(mapping.get_type());
            if (mappingMetaData == null) {
                logger.error("Not found the type: {} in the index: {}", mapping.get_type(), mapping.get_index());
                throw new IllegalArgumentException("Not found the type in the index: " + mapping.get_index());
            }
